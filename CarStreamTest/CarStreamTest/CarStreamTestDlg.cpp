...
// 		Sleep(40);
			...
			(Car4_tempFrame1 != NULL))
		{
			...
			//----------------车道2----------------//
			{
				...
				cvSmooth(Car2_tempFrame1, Car2_tempFrame1, CV_GAUSSIAN, 7,7);//平滑滤波
			...
			//----------------车道3----------------//
			{
				...
				cvSmooth(Car3_tempFrame1, Car3_tempFrame1, CV_GAUSSIAN, 7,7);//平滑滤波
				cvCvtColor(Car3_tempFrame1, Car3_currentFrame, CV_BGR2GRAY);//灰度转换

				if (Car3_imageCount==0)
					Car3_backgroundFrame = cvCloneImage(Car3_currentFrame);        //获取背景图像

				cvAbsDiff(Car3_currentFrame,Car3_backgroundFrame,Car3_diffFrame);  //绝对值做差值操作
